  ...
  return (
    <>
      ...
      <View style={componentStyles.titleContainerStyle}>
        <Text style={componentStyles.titleTextStyle}>{translate('phc.patient.list')}</Text>
        <TouchableOpacity
          ...
          <TouchableOpacity
            onPress={() =>
              navigation.navigate(ROUTES.PATIENT_DETAIL, {
                patientId: item.id,
                treatmentPlan: item.ongoingTreatmentPlan.length ? item.ongoingTreatmentPlan[0] : item.upcomingTreatmentPlan ? item.upcomingTreatmentPlan : item.lastTreatmentPlan,
              })
            }
            activeOpacity={0.7}
